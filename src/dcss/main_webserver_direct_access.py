...
def run_websockgame(game: WebSockGame):
    game.start()
...
if __name__ == "__main__":
    ...
    game_connect_thread = threading.Thread(target=run_websockgame, args=(game1,), daemon=True)
    game_connect_thread.start()
...
if __name__ == "__main__":
    ...
    config.species = ''
    config.game_mode = 'tutorial'

    game_connection = WebSockGame(config)


    while True:
        next_action = random.choice(commands)
        loop.call_soon(game1.send_action(next_action))
        gamestate = loop.call_soon(game_connection.get_game_state())
